...
resource "intersight_bios_policy" "platform-defaults" {
  name = "${var.custom-name}-platform-defaults"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "cpu-intensive" {
  name = "${var.custom-name}-cpu-intensive"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "io-intensive" {
  name = "${var.custom-name}-io-intensive"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "energy-efficient" {
  name = "${var.custom-name}-energy-efficient"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "low-latency" {
  name = "${var.custom-name}-low-latency"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "relational-db" {
  name = "${var.custom-name}-relational-db"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "virtualization" {
  name = "${var.custom-name}-virtualization"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "analytical-db" {
  name = "${var.custom-name}-analytical-db"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "data-analytics" {
  name = "${var.custom-name}-data-analytics"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
...
resource "intersight_bios_policy" "high-performance-computing" {
  name = "${var.custom-name}-high-performance-computing"
  dynamic "tags" {
    for_each = local.tags
    content {
      key   = tags.key
